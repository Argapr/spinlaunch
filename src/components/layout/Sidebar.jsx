...
import logoAI from "../../assets/chat-white.png"

const menuItems = [
    {
        icon: Coins,
        text: "Quick Launch Token",
    ...
    },
    {
        ...
        path: "/token-management/additional-tools",
        submenu: [{ text: "Create Staking Token", path: "/pools" }],
    },
    {
        icon: Database,
        text: "Token Manager",
        ...
        submenu: [
            {
                text: "Create OpenBook Market ID",
...
const Sidebar = () => {
    ...
    return (
        <aside className="bg-[#060136] text-white w-72 pt-10 p-5">
            <Link to="/chatbot">
                <button className="mb-4 w-full border border-[#fff] bg-purple-700 hover:bg-purple-800 text-white py-3 rounded-md font-medium flex flex-col items-center justify-center">
                    <img src={logoAI} alt="" className="h-auto w-10" />
                    ChatBot
                </button>
            </Link>
            <div className="mb-4">
                ...
                <SidebarItem
                    item={menuItems[0]}
                    active={location.pathname === menuItems[0].path}
                />
            </div>
                ...
                <div className="ml-4">
                    {item.submenu.map((subItem, index) => (
                        <Link to={subItem.path} key={index}>
                            <div
                                className={`py-2 px-4 rounded mb-1 cursor-pointer ${
